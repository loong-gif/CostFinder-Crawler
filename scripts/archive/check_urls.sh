...
check_url() {
    local url="$1"
    local idx="$2"
    local outfile="$results_dir/result_${idx}.txt"
    ...
    if [ $curl_exit -ne 0 ]; then
        ...
        echo "$url||ERROR||$error_msg||$result" > "$outfile"
        return
    fi
    ...
    echo "$url||$final_url||$http_code||$redirect_count||$problem" > "$outfile"
}
...
# Generate numbered URL list, pipe to xargs
awk '{print NR, $0}' "$url_file" | xargs -P 12 -I {} bash -c 'check_url "$(echo {})\" "$(cut -d\" \" -f1 <<< {})\""'
echo "ALL_DONE"
