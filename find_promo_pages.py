...
class PromoPageFinder:
    ...
    def find_promo_pages(self, domain: str) -> List[Dict]:
        ...
        base_url = URLValidator.normalize_url(domain)
...
def process_urls(input_file: str, output_file: str = None):
    ...
    # Normalize URLs and filter to keep only unique domains
    seen_domains = set()
    unique_urls = []
    ...
    for url in raw_urls:
        ...
        if domain not in seen_domains:
            ...
            print(f"⚠️ Skipping duplicate domain: {domain} (URL: {url})")
    
    print(f"\n📋 Total {len(raw_urls)} URLs read, filtered to {len(unique_urls)} unique domains")
    print("=" * 70)
