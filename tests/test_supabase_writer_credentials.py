...
def test_secret_key_alias_works_with_rollback_override(monkeypatch):
    monkeypatch.delenv("SUPABASE_WRITER_KEY", raising=False)
    monkeypatch.delenv("SUPABASE_SERVICE_ROLE_KEY", raising=False)
    monkeypatch.setenv("SUPABASE_SECRET_KEY", "sb-secret")
    monkeypatch.setenv("ALLOW_SERVICE_ROLE_WRITES", "true")
    assert get_supabase_writer_key() == "sb-secret"
